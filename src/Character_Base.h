...
protected:
	...
	float* anim_rate;	// それぞれのアニメーションのブレンド率（0.0f ~ 1.0f）
	...
	float* attack_anim_rate;   // それぞれの攻撃アニメーションのブレンド率（0.0f ~ 1.0f）
	...
	float* block_anim_rate;	       // それぞれのアニメーションのブレンド率（0.0f ~ 1.0f）
...
public:
	...
	// コンボ関連
	//---------------------------------------------------------------------------
	bool combo_flag;        // 近距離攻撃でのコンボフラグ
	int next_combo;         // 次に出してほしい攻撃
