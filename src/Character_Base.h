...
protected:
	...
	int action_mode = 0;  // 今攻撃モードなのか普通モードなのかを判別するを判断する用の変数

	//---------------------------------------------------------------------------
	// 普通アニメーション読み込み用の配列
	//---------------------------------------------------------------------------
	int* anim_model = 0;    // アニメーションモデル
	int* anim_attach = 0;   // モデルにアニメーションをアタッチする用の配列（アタッチ番号を取得する）
	float* anim_total = 0;  // アニメーションが何フレーム科を取得、それを保存する用の配列
	float* anim_rate = 0;	// それぞれのアニメーションのブレンド率（0.0f ~ 1.0f）
	float* anim_frame = 0;  // アニメーションが何フレーム進んでいるのか


	//---------------------------------------------------------------------------
	// 攻撃アニメーション用変数
	//---------------------------------------------------------------------------
	int* attack_anim_model = 0;    // 各攻撃アニメーションアニメーション読み込み用の配列
	int* attack_anim_attach = 0;   // モデルに攻撃アニメーションをアタッチする用の配列（アタッチ番号を取得する）
	float* attack_anim_total = 0;  // 攻撃アニメーションが何フレーム科を取得、それを保存する用の配列
	float* attack_anim_rate = 0;   // それぞれの攻撃アニメーションのブレンド率（0.0f ~ 1.0f）
	float* attack_anim_frame = 0;  // 攻撃アニメーションが何フレーム進んでいるのか


	//---------------------------------------------------------------------------
	// ガードアニメーション用変数
	//---------------------------------------------------------------------------
	int* block_anim_model = 0;         // アニメーションモデル
	int* block_anim_attach = 0;        // モデルにアニメーションをアタッチする用の配列（アタッチ番号を取得する）
	float* block_anim_total = 0;       // アニメーションが何フレーム科を取得、それを保存する用の配列
	float* block_anim_rate = 0;	       // それぞれのアニメーションのブレンド率（0.0f ~ 1.0f）
	float* block_anim_frame = 0;       // アニメーションが何フレーム進んでいるのか


	//---------------------------------------------------------------------------
	// ダメージ（受けた時の）アニメーション用変数
	//---------------------------------------------------------------------------
	int* damage_anim_model = 0;    // アニメーションモデル
	int* damage_anim_attach = 0;   // モデルにアニメーションをアタッチする用の配列（アタッチ番号を取得する）
	float* damage_anim_total = 0;  // アニメーションが何フレーム科を取得、それを保存する用の配列
	float* damage_anim_rate = 0;   // それぞれのアニメーションがどのくらうのわりあいになるか（0.0f ~ 1.0f）
	float* damage_anim_frame = 0;  // アニメーションが何フレーム進んでいるのか
...
public:
	int attack_anim_pick = 0;     // どのアニメーションをしているのかを判断する用
	int block_anim_pick = 0;      // どのアニメーションをしているのかを判断する用
	int damage_anim_pick = 0;     // どのアニメーションをしているかを判断する用の変数

public:
	//---------------------------------------------------------------------------
	// 各キャラの当たり判定用変数
	//---------------------------------------------------------------------------
	bool cd_hit_flag = false; //< あたり判定を行てよいかのフラグ


	//---------------------------------------------------------------------------
	// 各アクション判断用変数
	//---------------------------------------------------------------------------
	bool action_flag = false;       // アクション（何かのアニメーション）されているかのフラグ
	bool attack_flag = false;       // 今攻撃中なのかの判断 
	bool damage_flag = false;       // 今ダメージを受けているのか
	bool block_flag = false;        // 今ガード中なのか
	bool bead_hit_flag = false;     // 弾が何かにあたったか
	bool skill_flag = false;        // スキル技の使用できるかのフラグ
	bool sp_flag = false;           // 必殺技の使用できるかのフラグ

	//---------------------------------------------------------------------------
	// コンボ関連
	//---------------------------------------------------------------------------
	bool combo_flag = false;        // 近距離攻撃でのコンボフラグ
	int next_combo =0;         // 次に出してほしい攻撃
	...
	// 投げものの当たり描画用変数（円）
	//---------------------------------------------------------------------------
	Vector3 bead_pos = { 0.0f,0.0f,0.0f };  // 座標
	float   bead_r = 0.0f;    // 半径
...
protected:
	...
	Vector3 mov = { 0.0f,0.0f,0.0f };          //  ゲームパッド用移動用ベクトル用変数
public:
	Vector3 before_mov = { 0.0f,0.0f,0.0f };   // 毎フレーム移動前の座標をいれる用の変数
	...
	bool skill_cooldown = false;  // スキル使用後のクールダウン
	bool SP = false;              // 必殺技のSP
	// ↑上の二つの変数の上限は各キャラのクラスで定義する
	//---------------------------------------------------------------------------
	// 攻撃時の当たり判定用変数
	//---------------------------------------------------------------------------
	Vector3 hit_pos_pot = { 0.0f,0.0f,0.0f };
	Vector3 hit_pos_under = { 0.0f,0.0f,0.0f };
};